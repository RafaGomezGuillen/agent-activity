...
export class AgentDetailComponent implements OnInit {
  ...
  loadAgent(): void {
    this.agentService.getAgent(this.agentId).subscribe({
      next: (a) => { this.agent = a; this.loading = false; this.cdr.detectChanges(); },
  ...
  buildChart(): void {
    ...
    this.chartOptions = {
      ...
      scales: {
        x: { ticks: { color: '#475569', font: { family: 'monospace', size: 10 } }, grid: { color: '#1e293b' } },
        y: { min: 0, max: 100, ticks: { color: '#475569', font: { family: 'monospace', size: 10 } }, grid: { color: '#1e293b' } },
      },
    };
  }
}
